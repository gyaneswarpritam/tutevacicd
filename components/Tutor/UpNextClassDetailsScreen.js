...
const UpNextClassDetailsScreen = ({ route, navigation }) => {
  ...
  const AboutClass = () => (
    <View
      ...
      {Object.keys(classList).length > 0 ? <ScrollView
        ...
        {classList?.timings &&
          classList?.timings?.length > 0 &&
          classList.timings.map(res => {
            return (
              <Text
                key={res._id}
                style={{
    ...
    videoCall ? (
      ...
      <View style={{ flex: 1, backgroundColor: '#F6F6F6' }}>
        <Appbar.Header style={MainStyleSheet.navBar}>
          <Appbar.Action
            icon="arrow-left"
            onPress={() => navigation.navigate('UpNextClass')}
          />
...
export default UpNextClassDetailsScreen;
